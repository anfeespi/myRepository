...
/**
 * 
 * Panel principal de la ventana de elecci�n del jugador
...
public class PanelPersonajes extends JPanel {
	...
	/**
	 * Etiqueta con el texto "Personaje" que indica la selecci�n del personaje
	 */
	 ...
	 * Personajes a elegir
	 */
	private String[] lista_personajes = {"Alien", "Robot", "Astronauta", "Fantasma"};
	/**
	...
	public void init() {
		...
		try {
			bi2 = ImageIO.read(new File("src/img/img_next.png"));
			Image redimensionado2 = bi2.getScaledInstance(next.getWidth(), next.getHeight(), Image.SCALE_SMOOTH);
			next.setIcon(new ImageIcon(redimensionado2));
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		try {
			bi3 = ImageIO.read(new File("src/img/img_back.png"));
			Image redimensionado3 = bi3.getScaledInstance(back.getWidth(), back.getHeight(), Image.SCALE_SMOOTH);
			back.setIcon(new ImageIcon(redimensionado3));
		} catch (IOException e) {
			e.printStackTrace();
		}
